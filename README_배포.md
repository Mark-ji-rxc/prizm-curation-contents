...
**이미지 찾기 ④**
- **가로형/세로형 구분 표시**: 실제 이미지 비율로 판별해 가로형은 썸네일을 꽉 채우고, **세로형은 좌우 여백을 두고 온전히 노출 + 좌상단 '세로' 배지** → 한눈에 방향 구분
- **이미지 없이도 미리보기로 진행 가능**(상품/쇼룸만 등록 시 이미지 불필요). 형태 검증은 ⑤에서.

**미리보기 ⑤**
- **① 이미지+상품/쇼룸 카드**: 이미지 **순서변경(◀ ▶)·삭제(✕)** — 발행 시 이 순서대로 업로드
- **② 상품/쇼룸만 카드**: 상품/쇼룸 **순서변경·삭제** — 발행 시 이 순서대로 조회·추가
- 순서/삭제는 **등록 대기목록 → 백오피스 발행까지 그대로 반영**
- **발행 형태 검증**: ① 이미지+상품/쇼룸 · ③ 이미지↔상품/쇼룸 매칭 선택 시 이미지가 없으면 알림(이미지 없이 등록하려면 ② 상품/쇼룸만 선택)
