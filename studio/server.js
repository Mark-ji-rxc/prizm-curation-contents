...
const CONTENT_RULES = {
  ...
  톤: '친구에게 말 걸듯 친근·다정하게. 딱딱한 격식체(~입니다/~습니다/~하십시오/~좋을까요?) 금지, 친근한 반말이나 부드러운 해요체(~해요/~거든요/~봐요) 사용. ★ 한 콘텐츠 안에서는 반말 또는 해요체 하나로 일관(섞지 말 것). 어미·문장 시작·길이 변주(기계적 반복 금지).',
...
function buildContentJob({ topic, count, perTopic, forms, scope, region, form, persona, condition, until, productCodes, productTypes, mode, userTitle, userBody, model, brief, webSearch, bodyMin, bodyMax, regionMode, festival }) {
  ...
  if (mode === 'match') {
    ...
    const regionsLine = regionCands.length === 1
