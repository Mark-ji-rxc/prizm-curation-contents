...
- **2026-09-23 (④ 이미지 목록 TTL 캐시 — 재진입 즉시)**: 병렬화로 남은 1.8초도 없애달라는 요청(1번안). **⛔ 제약**: NAS에는 절대 쓰지 않는다 — 캐시는 **서버 프로세스 메모리(Map)에만** 둔다(디스크·NAS 기록 없음). 작업 전후로 읽기전용 재감사: 호출 API는 `List`/`Search`/`Download`/`Thumb` 뿐, `Upload`·`Delete`·`CreateFolder`·`Rename`·`CopyMove`·`Edit` 호출 **0건** 확인. **구현**: `listImagesRecursive(root, {ttlMs=10분, useCache=true})` — `root|maxDepth|cap` 키로 결과 재사용, 반환은 복사본(호출자가 배열을 바꿔도 캐시 미오염), 항목 50개 상한. studio 서버의 `/api/images`·`/api/list-images`가 `fresh=1`이면 캐시를 우회하도록 연결. UI에 **[↻ 새로고침] 버튼** 추가(`#imgRefresh` → `loadImages(target, true)`) — NAS에 새 이미지를 올린 직후에 쓴다. **실측(브라우저)**: 재진입 **52ms**(이전 1.8~2.0초), 강제 새로고침 1519ms, 결과 동일(311장). API 기준 1회차 1.85초 → 2회차 **20ms**. ※ 절충안: NAS 변경은 최대 10분 뒤 반영(즉시 보려면 새로고침 버튼).
...
- **2026-09-22 (콘텐츠 생성 속도 개선 — 생성 샤딩 + products 슬림화 + 실패 표시)**: 생성이 너무 느리다는 요청으로 job 기록(usage.durationMs)을 실측해 병목을 특정. **실측**: content job 중앙값 598초, 20편이면 1213~1878초(20~31분), 캐시읽기 최대 8.6M 토큰, job 파일 1MB(products 522KB/1,334건), `claude -p` 기동 오버헤드는 ~4초(무시 가능 → 샤딩 유리). **세 가지 조치**: ① **생성 샤딩(plan→write)** — 한 job이 N편을 순차로 쓰던 걸 2단계로 분리. 1단계 `phase:'plan'`이 전체 상품을 보고 주제 N개를 한 번에 기획(주제 다양성·중복방지를 전역 1회로 유지 = 품질 보존), 2단계 `phase:'write'`가 주제를 K개로 쪠개 동시 집필(`SHARD_MIN_COUNT=6`, `SHARD_PER_JOB=5`, `SHARD_MAX=4`). `watchPlanAndFanOut()`이 plan 완료를 감지해 write job들을 생성·병렬 dispatch하고 plan의 `output.shardJobs`에 기록. `/api/content/job`이 샤드를 **집계**해 하나처럼 응답(status/items/usage 합산, 벽시계는 기획+최장샤드)하므로 클라이언트 변경 최소. write job은 배정 주제의 상품만 실어 payload가 556KB→33KB(94%↓). count<6이거나 지역모드면 기존 단일 job 그대로. ② **products 슬림화**(`compactForJob`) — `url` 제거(productCode에서 파생), `flags`는 true인 항목만 배열로, 빈 값 생략 → 554KB→313KB(43%↓, 142K→80K 토큰). 스키마 변경을 지시문에 명시. 출력 스펙에서도 미사용 `url` 제거(4곳). ③ **실패 표시**(`markJobFailed`) — 예전襌 실패해도 job이 `pending`으로 남아 UI가 무한 스피너였음(오늘 크레딧 소진 때 실제 발생). 이제 spawn 실패·`is_error`·비정상 종료·결과 미기록을 모두 `status:'failed'`+`error`로 기록하고, 클라이언트(`pollContent`/`pollBriefJob`)가 즉시 사유를 표시. 진행률(주제 기획 중/집필 n/m)도 배너에 노출. **검증(크레딧 없이)**: 포트 8799/8798 테스트 서버에서 count=20 → `sharded:true, shards:4`, plan 결과 주입 시 write job 4개(각 5주제·15상품·33KB) 팬아웃, 완료 집계 20편·usage 합산·벽시계 350초, 샤드 실패 시 `failed`+사유 전파, 실제 크레딧 에러도 `failed`로 기록 확인. 테스트 job은 전량 정리. ※ 모델·웹검색 변경은 품질 우선으로 **미적용**(사용자 요청).
  **[추가] 구독 전환 + 실제 검증**: `claude` CLI가 키체인 API 키(종량제)로 돌고 있어 크레딧 소진 시 생성이 즉시 실패했고, 작업당 $3~12이 실제 과금되고 있었다(주석의 '구독으로 동작'과 달람). `/login`으로 구독 전환 완료 (`billingType: stripe_subscription`, `organizationType: claude_pro`, API 키는 키체인에서 제거됨). **실측(6편·샤드 2)**: 총 339초(기획 ~176초 + 집필 병렬), 6편 모두 서로 다른 본문형·화자·주제로 샤드 간 중복 없음을 확인(기획 전역 1회 설계가 의도대로 동작). **[버그수정] done-before-items 경합**: 에이전트가 `status:"done"`을 먼저 쓰고 `items`를 나중에 써서, 그 순간 집계하면 **0편으로 완료 확정**되는 문제(폴링이 done에서 멈췄). 실제 검증 중 발생. 수정: 샤드 완료 판정을 `status==='done' && items.length>0`로 강화, 단일 job도 빈 배열이면 미완으로 취급, 프로세스 종료 후 결과가 비어 있으면 `failed`로 표시(`markJobFailed(..., force)`). 경합 시뮬레이션으로 검증.


- **2026-09-15 (필터 키워드 유실 수정 — 한글 IME Enter + 잔여 입력 플러시)**: ⑥ 편집기에서 필터 키워드를 입력해도 발행 시 등록이 안 되던 문제. **원인**: 발행측 로직(`publisher.js` `setFilterKeywords`/`selectKeywordOption`/신규 `registerKeyword`)은 실측 정상(기존 키워드 "일본"·"하노이" 선택, 옵션형식 `"<이름> (N개)"`, 톱니→모달 placeholder "키워드명을 입력해주세요."+등록/닫기 정상). 하지만 **UI 수집 단계에서 유실** — 큐 54건 전부 `filterKeywords:[]`. 한글 IME 조합 중 Enter는 IME가 먼저 소비(`isComposing`/keyCode 229)해 칩 추가 핸들러가 첫 Enter에 안 걸림(합성 이벤트 테스트만 통과했던 이유). **수정(app.js)**: ① `addPubKeyword(it,name)` 헬퍼(중복 방지), ② Enter 핸들러에 `e.isComposing||keyCode===229` 가드, ③ **`collectPubItem`에서 `#pf-kw-input`에 남은 텍스트를 저장/발행 시 자동 반영**(Enter 안 눌러도·IME로 놓쳐도 유실 방지). **검증(8790)**: Enter 없이 "호캉스" 입력→저장 시 반영·입력창 비움, 중복 '일본' 무시, IME Enter('제주') 무시 후 잔여로 보존, 서버 저장·재조회 확인. (draft 테스트값 원복)

- **2026-09-15 (쇼룸 노출 발행 버그 2건 수정 — 발행주체 쇼룸 오선택 + 아이템 쇼룸 미등록)**: 쇼룸으로 발행 시 ① 발행 주체가 "체크인/인트립"이 아니라 엉뚱한 쇼룸("경주 체크인불국")으로 들어가고 ② 아이템 쇼룸이 아예 등록 안 되던 문제. **원인**(publisher.js): ① 쇼룸 옵션 텍스트가 `"<id>\n<이름>"`(id와 이름 사이 **줄바꿈**)이라 기존 정규식 `\d+\s+체크인$` 매칭 실패 → substring `.first()` 폴백이 "체크인"을 포함한 **기존 쇼룸** "경주 체크인불국"을 잘못 선택(※ 신규 생성 아님, 원래 있던 쇼룸을 오선택). ② 아이템 쇼룸 등록은 미구현(`// 후속 확장`). **수정**: `selectShowroomOption(page,input,name)` 신설 — 드롭다운의 **기존 옵션만**, 선두 id 제거 후 이름 **완전일치**(공백무시 폴백), 없으면 false→에러(⚠️ **절대 신규 쇼룸 생성 안 함**). 발행주체 쇼룸 선택을 이걸로 교체. 아이템 쇼룸: `item.items` 중 `kind==='showroom'`이면 아이템 **'쇼룸' 탭**(role=tab) 클릭 → `쇼룸 선택` 드롭다운에서 각 쇼룸 선택(선택 즉시 표에 행 추가, 추가 버튼 없음). 못 찾으면 미등록 방지로 에러. `buildPublishDraft`는 이미 `publisherShowroom`=도메인 기준 체크인/인트립·`items`=showroomName으로 정상. **검증(프로덕션 폼, 저장X)**: 발행주체 "체크인" 정확 선택(칩='체크인'), 아이템 "JW 메리어트 호텔 하노이" 표 추가, 없는 쇼룸명은 false·표 무변화(생성 안 됨) 실측. ※ **서버 재시작 필요**(publisher.js는 서버 기동 시 require).

- **2026-09-14 (백오피스 공개중 콘텐츠 학습 → 생성 품질 향상)**: 프로덕션 백오피스(manager-office.prizm.co.kr) Discover 게시글 중 **공개중(PUBLIC) 60편**의 제목·본문(이모지 포함)을 **읽기전용 API**로 수집(`/manager/discover/post/search` 목록 + `/manager/discover/post/{id}` 상세, 세션 토큰은 `office-session-prod.json`. ※백오피스는 조회만, 쓰기·수정 금지 원칙). → **노션 코퍼스 DB** 생성·업로드(60건, 유형 SHOWROOM 36·PROFILE 18·REVIEW 6 / 국내 34·해외 26): https://app.notion.com/p/fdde0bbf38834dca9a62dc3752428a3f (data_source_id `6eea9623-37c6-4b6d-b7f0-71815b716a1a`). → 고객 원문인 **"내돈내산 리얼 리뷰"(REVIEW 6편)는 우리 말투가 아니므로 학습 제외**, 에디터 작성 54편(SHOWROOM+PROFILE)만 분석. **학습 결과 2곳 반영**: ① `저장콘텐츠_학습분석.md`(사람이 보는 학습 문서 — 제목 도출 방법론 9유형+5단계 공식, 톤·이모지, 이미 다룬 주제 목록, 새 주제 도출법) ② `saved-insights.json`(생성 프롬프트에 실제 주입되는 `principles[]` 15개 — 제목 최우선. server.js `loadSavedInsights()`→`insLine`으로 generate/brief/region 지시문에 주입). **핵심 학습**: 제목은 상품·가격을 감추고 호기심·공감·반전·숫자 훅으로, 1줄 6~20자(중앙값 17)·필요시 2줄(훅/대상), 해요체 하우스 보이스, 본문 이모지 편당 1~3개. ⚠️ 스튜디오에서 "저장 콘텐츠 학습 분석" 잡을 다시 돌리면 `saved-insights.json`이 저장콘텐츠 기준으로 덮어써짐(이번 값은 공개중 콘텐츠 기준, `source`/`count`:54로 표기).
...
- **2026-09-07 (지역/축제 콘텐츠 상품 매칭 + 어투 일관성)**: ① **지역·축제 콘텐츠도 matched 채움**: 기존엔 지역모드가 matched 빈배열이었으나, 이제 buildContentJob이 대상 지역 상품 풀(`regionProducts`: 선택 지역이면 그 지역 selling 상품, 전체면 스코프 풀)을 job products로 전달하고, 지시문을 "본문은 지역/축제 중심(상품 나열·광고·가격 금지)이되 그 지역(축제면 개최지·인근 소도시)과 지리적으로 맞는 상품을 matched에 최대한 연결, 억지매칭 금지·없으면 빈배열"로 변경. output 형식 matched를 상품 객체로, rules.상품매칭도 갱신. 검증(8793): 나트랑 지역+축제 job에 products 12개·상품매칭 지시·matched 예시 포함. ② **어투 일관성**(요청): `toneRule` 추가 — 한 콘텐츠 안에서는 반말/존댓말 중 하나로만 끝까지 통일(섞지 말 것)을 generate·brief·region 3모드 지시문 + CONTENT_RULES.톤 + 제작가이드 md에 반영(기존 "반말+존댓말 마무리 혼용" 문구 제거).

- **2026-09-07 (발행 대기목록 → 노션 리스트업, 국내/해외 분리)**: ⑥ 등록에 **[📋 노션 리스트업]** 버튼 — 발행 대기목록을 노션 DB로 내보내 사람이 검토·컨펌(상태 '편성 확정')하도록. 예시 DB(국내탭 게시글 검토) 스키마 그대로 + **상품 ID(숫자) 컬럼 추가**. **국내/해외 DB 각각**(요청): domain overseas→해외 DB, domestic·common→국내 DB로 라우팅. 서버 `notionRowsFromQueue()`(게시글 제목/상태/게시글 유형[미디어·노출로 판정]/발행 형태/발행쇼룸/본문/게재 기간/상품 ID/상품·콘텐츠·쇼룸 아이디/키워드/도메인), `buildNotionExportJob()`이 `notion-sync.json`의 두 DS를 참조해 job 생성, `dispatchToClaude(id,'notion')`로 Claude(MCP)가 upsert(제목 중복 갱신). 엔드포인트 `POST /api/publish/notion-export`·`GET /api/publish/notion-export/job`(완료 시 DB정보 notion-sync.json 저장). Notion MCP로 국내/해외 DB 2개 생성+현재 큐 4건(국내3·해외1) 채움. notion-sync.json gitignore(워크스페이스별). 검증(8793): export job 라우팅·rows domain·양 DB url 정상, 버튼 렌더. ※ 최초 통합 DB(e680243a)는 국내/해외 분리 전 생성분 — 삭제 예정.

- **2026-09-07 (버튼 한 번 로그인 + 노출 라벨 한 줄)**: 터미널 명령 없이 **스튜디오 버튼으로 백오피스 재로그인**. 서버 `startOfficeLogin(env)`가 헤드풀 Chromium을 띄우고 로그인 완료(localStorage 유효 토큰 등장, 만료>지금+1분)를 1.5초 폴링→감지 시 `storageState`로 세션 저장(해당 환경 캐시 무효화). 엔드포인트 `POST /api/publish/login?target=`(시작)·`GET /api/publish/login/status?target=`(running/done/failed, 최대 6분·창 닫힘 감지). UI: 캘린더 세션경고 배너에 **[🔓 스튜디오에서 바로 로그인]** 버튼 → `officeLogin()`이 상태 폴링, 완료 시 "✅ 완료→새로고침"하고 loadCalendar(true)+renderSessionWarn. 터미널 방식은 보조 안내로 유지. **부가**: 발행 에디터 전시순서/기간 행의 체크박스 라벨(노출·무기한)이 좁은 폭에서 2줄로 깨지던 것 `.pf-row .row.gap label{white-space:nowrap}`로 한 줄 고정. 검증(8790): login/status idle, 만료 배너에 로그인 버튼 렌더, pf-visible 라벨 white-space=nowrap.

- **2026-09-07 (최근 생성 결과 복원)**: 새로고침·재접속 후에도 완료된 생성 결과를 다시 불러오는 기능. 서버 `GET /api/content/recent?limit=`(jobs/ 최근 content 잡을 createdAt 역순, 라벨=브리프/지역/자동주제·status·count 반환). UI: 콘텐츠 배너 아래 "🕘 최근 생성 결과" 토글 패널(`#recentPanel`) — 잡별 상태(생성 중/완료/실패)·라벨·편수·상대시간 + **[불러오기]**(done만), **"완료 N건 모두 불러오기"**, 새로고침. `loadRecentJob`이 `/api/content/job?id=`로 items 받아 genItems에 누적(중복방지 loadedRecent Set→"불러옴" 표시), 진행중 잡 있으면 패널 열려있을 때 3초 자동 새로고침(→배치가 새로고침 후에도 끝나면 여기서 확인·복원). 검증(8790): recent 30건·완료 19 로드가능, 첫 잡(지역 20편) 불러오기→카드 20장 누적, 재클릭 시 "불러옴" 중복방지.

- **2026-09-07 (브리프 배치 — 여러 건 동시 지시)**: 브리프 모드를 **여러 건 한 번에 지시**하도록 개편(순차 입력→목록). `#briefList`에 브리프 행 여러 개(각 텍스트+구분 select[현재/국내/해외]), "+ 브리프 추가"(최대 12), "브리프 전체 생성". 서버 변경 없음 — 각 브리프를 기존 `/api/content/generate`(mode brief)로 **병렬 디스패치**(claude -p 각각), `pollBriefJob`이 완료되는 대로 결과를 `genItems`에 **누적**해 카드 추가(단건 생성은 교체, 배치는 append). 진행 패널(`#briefBatch`): 브리프별 대기/생성중/완료/실패 + "완료 X/전체 Y". `renderContentCards`를 모듈 배열 `genItems` 기반으로 리팩터(인자 주면 교체=하위호환). 브리프는 전체 상품 풀 사용이라 updateGenGate 대상에서 제외. 탭 열어둔 채 다른 작업 가능(결과는 카드에 누적). 검증(8790): 3건(도쿠시마/대만 등불/대관령 억새) mock 배치→완료 3/3·카드 3장 누적, +추가/삭제·구분 select 정상. ⚠️ 페이지 새로고침 시 진행중 배치 추적 유실(잡은 서버에서 계속 생성됨) — 향후 필요 시 최근 생성 잡 목록 복원 고려.

- **2026-09-07 (프로덕션 발행 + 지역 축제 콘텐츠)**: **① 프로덕션 등록**: 백오피스 발행을 환경(stage/prod)별로. `_officecfg.js`에 `envConfig(env)` 추가 — stage=manager-office-stage/`office-session.json`, prod=manager-office/`office-session-prod.json`(+apiBase 자동유추). publisher `publishItem`이 `item.target`으로 환경 선택, publish-login.js는 `node publish-login.js prod`로 프로덕션 세션 저장. office-posts(fetchOfficePosts/sessionInfo)·엔드포인트(office-status/office/posts/office/session)에 `?target=` 지원(캐시도 환경별). UI: 발행 대기 에디터에 **발행 대상(스테이지/프로덕션)** 라디오(프로덕션=빨강 강조), runPublish가 대상 세션 점검 + **프로덕션은 강한 확인창**. 캘린더에도 스테이지/프로덕션 세그(환경 전환 시 재조회). buildPublishDraft `target:'stage'` 기본. .gitignore에 office-session-prod.json. 검증(8790): envConfig stage/prod 경로·apiBase, office-status prod sessionOk=false·baseUrl manager-office, UI 라디오/세그 렌더. **② 지역 축제 콘텐츠**(지역모드 하위옵션): `#cFestival` 체크 시 buildContentJob이 **검색시점~4개월 이내** 개최 축제(대상지역+소도시·시군읍면 단위 포함)를 WebSearch로 상세 조사해 소개하도록 지시(축제 없으면 계절매력 대체·지어내기 금지·미확인 세부는 speculative). commonBody `festival`(지역모드에서만), 지역모드 OFF 시 축제 자동 비활성. 검증(8793): festival job window 2026-09-07~2027-01-07·소도시 검색 지시·webSearch 강제, UI 토글 enable/disable 정상.
